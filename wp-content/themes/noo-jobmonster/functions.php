...
if(! function_exists('social_apply_linkedin')):
    function social_apply_linkedin(){
        if(isset($_GET['apply']) && ($_GET['apply']=='linkedin')) {
            $apply = new Noo_Apply_Linkedin();
            $apply->oauth();
        }
    }
endif;
add_action('wp_loaded','social_apply_linkedin', 20);

function noo_open_linkedin_apply_popup(){
    if(isset($_SESSION['open_popup_apply_linkedin']) && $_SESSION['open_popup_apply_linkedin']){
        noo_get_layout('job/apply/via_linkedin_form_popup');
        ?>
        <script type="text/javascript">
            jQuery(window).on('load',function(){
                jQuery('#applyJobviaLinkedInModal').modal('show');
            });
        </script>
        <?php
        unset($_SESSION["member_linkedin"]);
        unset($_SESSION["open_popup_apply_linkedin"]);
    }
}
// add_action('wp_footer','noo_open_linkedin_apply_popup');
