...
if ( ! function_exists( 'jm_set_resume_expired' ) ) :
	function jm_set_resume_expired( $resume_id = '' ) {
		...
		if ( empty( $_ex ) && $package = jm_get_resume_posting_info( $candidate_id ) && $post_resume_role == 'package' ) {
			$package_interval      = absint($package['package_interval'] );
			$package_interval_unit = $package['package_interval_unit'];
			$_expires              = strtotime( "+{$package_interval} {$package_interval_unit}" );
