    ...
    function jm_message_cannot_download_cv_candidate(){
        ...
        switch( $can_download_cv ) {
            ...
            case 'employer':
                ...
                break;
            case 'package':
                $message = __('Only paid employers can download candidate\'s CV.<br />','noo');
	...
	function jm_job_package_view_candidate_contact_data() {
		...
		if( jm_is_enabled_job_package_view_candidate_contact() ) {
            ...
            woocommerce_wp_text_input( array(
                ...
                'label'             => __( 'View Candidate\'s Contact Limit', 'noo' ),
                'description'       => __( 'The number which user can view candidate\'s contact with this package. Input -1 for unlimited.', 'noo' ),
	...
	function jm_job_package_view_candidate_contact_features( $product ) {
		if( jm_is_enabled_job_package_view_candidate_contact() && $product->get_can_view_candidate_contact()) :
            ...
            <?php if( $view_candidate_contact_limit == -1 ) : ?>
                <li class="noo-li-icon"><i class="fa fa-check-circle"></i> <?php _e('View Candidate Contact Unlimited ','noo');?></li>
            <?php elseif( $view_candidate_contact_limit > 0 ) : ?>
                <li class="noo-li-icon"><i class="fa fa-check-circle"></i> <?php echo sprintf( _n( '%d View Candidate\'s contact', '%d Views Candidate\'s contact',$view_candidate_contact_limit, 'noo' ), $view_candidate_contact_limit ); ?></li>
            <?php endif; ?>
    	<?php endif;
	}
		...
		if(is_array( $package ) && isset( $package['product_id'] ) && !empty( $package['product_id'] ) ) {
			...
			if( jm_is_enabled_job_package_view_candidate_contact() && $package['can_view_candidate_contact']) : ?>
                ...
                <?php if(  $view_candidate_contact_limit ) : ?>
                    <div class="col-xs-6"><strong><?php _e(' View Candidate\'s contact Limit','noo')?></strong></div>
                    <?php if( $view_candidate_contact_limit == -1) : ?>
