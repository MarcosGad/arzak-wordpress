...
//                                if($key == '_noo_job_field_filter_distance'){
//                                    continue;
//                                }
		...
		private function _format_email( $alert, $user, $jobs ) {
			...
			if ( $jobs && $jobs->have_posts() ) {
				...
				while ( $jobs->have_posts() && $count <= $max_alert_job_count ) :
					...
					$message .= sprintf(__('To cancel the notification of receiving information, please <a href="%s">login the website</a> and remove them.','noo'),Noo_Member::get_endpoint_url( 'job-alert' )). '<br/>';
					$message .= __( '------', 'noo' ) . '<br/>';
