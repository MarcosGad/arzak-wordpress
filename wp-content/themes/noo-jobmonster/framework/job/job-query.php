    ...
    function jm_job_query_from_request( &$query, $REQUEST = array() ) {
        ...
        foreach ( $tax_list as $term ) {
            ...
            if ( isset( $REQUEST[ $tax_key ] ) && ! empty( $REQUEST[ $tax_key ] ) ) {
                ...
                // } else {
                //     $tax_query_temp =  array( 'relation' => 'OR' );
                //     foreach ($REQUEST[$tax_key] as $value) {
        ...
        foreach ( $tax_list as $term ) {
            ...
            if (isset($request[$tax_key]) && !empty($request[$tax_key])) {
                ...
                if (!is_array($request[$tax_key])) {
                    ...
                    $url_back = remove_query_arg($tax_key,$_SERVER['HTTP_REFERER']);
                    ...
                    foreach ($request[$tax_key] as $key=>$value) {
                        /*	$page_archive = get_post_type_archive_link('noo_job');
                            $url   = add_query_arg(array($request),$page_archive);*/
        ...
        foreach ( $job_fields as $field ) {
            ...
            if (isset($request[$field_id]) && !empty($request[$field_id])) {
                ...
                if (is_array($value)) {
                    ...
                    $url_back = remove_query_arg($field_id,$_SERVER['HTTP_REFERER']);
